<template>
	<div class="task-detail-page">
		...
		<div v-if="task" class="task-detail">
			...
			<div class="detail-row"><span class="label">计划时间：</span><span class="value">{{ task.planStartTime }} 至
					{{ task.planEndTime }}</span></div>
			<div class="detail-row"><span class="label">实际时间：</span><span
					class="value">{{ task.actualStartTime || '未开始' }} 至 {{ task.actualEndTime || '未结束' }}</span></div>
			<div class="detail-row"><span class="label">巡检线路：</span><span class="value">{{ task.line }}</span></div>
			<div class="detail-row"><span class="label">巡检范围：</span><span class="value">{{ task.inspectionArea }}</span>
			</div>
			<div class="detail-row"><span class="label">任务状态：</span><span class="value">{{ task.status }}</span></div>
			<div class="detail-row"><span class="label">完成度：</span><span class="value">{{ task.completion }}%</span>
			</div>
			<div class="detail-row"><span class="label">发现问题：</span><span class="value">{{ task.issues }}个</span></div>
			<div class="detail-row"><span class="label">任务描述：</span><span class="value">{{ task.description }}</span>
			</div>
		</div>
