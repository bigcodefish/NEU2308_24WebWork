		...
		<div class="wireframe">
			<form class="search-form" @submit.prevent="onSearch">
				...
				<div class="form-item">
					...
					<input id="minDistance" v-model.number="filters.minDistance" type="number" min="0" step="0.1"
						placeholder="最小距离" />
				</div>
				<div class="form-item">
					<label for="maxDistance">最大距离(m)</label>
					<input id="maxDistance" v-model.number="filters.maxDistance" type="number" min="0" step="0.1"
						placeholder="最大距离" />
				</div>
				<div class="form-item">
				...
				<table class="table">
					...
					<tbody>
						...
						<tr v-for="(item, index) in paginatedData" :key="item.id">
							...
							<td class="ellipsis-cell" :title="item.defectNo || 'DEF-' + item.id">
								{{ item.defectNo || 'DEF-' + item.id }}</td>
							...
							<td>
								<span :class="['tag', item.isReal ? 'tag-success' : 'tag-danger']">
									{{ item.isReal ? '是' : '否' }}
								</span>
							</td>
							<td>{{ item.severity || '-' }}</td>
							...
							<td class="actions">
								<div class="action-buttons">
									<!-- 状态流转按钮 -->
									<span v-if="item.status === '待确认' && !item.isReal" class="link"
										@click="confirmReal(item)">
										确认属实
									</span>

									<span v-if="item.status === '已确认'" class="link" @click="startProcessing(item)">
										开始处理
									</span>

									<span v-if="item.status === '处理中'" class="link" @click="markFixed(item)">
										标记整改
									</span>
			...
			<div class="modal-content">
				...
				<div class="modal-image">
					<div class="image-container" ref="imageContainer" @wheel.prevent="handleWheel"
						...
						<img v-else :src="getImageUrl(modalData.imageUrl)" alt="缺陷大图" ref="modalImage" draggable="false"
							:style="{
                   ...
                   cursor: isDragging ? 'grabbing' : imageScale > 1 ? 'grab' : 'default'
					...
					<div class="image-controls" v-if="modalData.imageUrl">
						...
						<button class="control-btn" @click="downloadImage" :disabled="downloading">
							{{ downloading ? '下载中...' : '下载' }}
						</button>
		...
		<div v-if="remarkModalVisible" class="modal" @click.self="closeRemarkModal">
			<div class="modal-content" style="width: 500px;">
				...
				<div class="form-item" style="width: 100%;">
					<textarea v-model="remarkContent" placeholder="请输入处理备注..." rows="6"
